...
static int enetc_phylink_create(struct enetc_ndev_priv *priv)
{
	...
	mdiodev = mdio_device_create(pf->bus, 0);
	if (!mdiodev) {
		dev_err(dev, "cannot create PCS mdio device\n");
		goto imdio_remove;
	}

	pf->pcs = mdio_lynx_pcs_create(mdiodev);
	if (!pf->pcs) {
		dev_err(dev, "cannot create Lynx PCS device\n");
		goto imdio_remove;
	}

	pf->phylink_config->dev = pf->si->ndev;
