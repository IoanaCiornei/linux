				    ...
				    struct phylink_link_state *state)
{
	...
	mac->pcs->get_state(mac->pcs->dev, mac->if_mode, state);
}
...
static void dpaa2_mac_pcs_an_restart(struct phylink_config *config)
{
	...
	mac->pcs->an_restart(mac->pcs->dev, mac->if_mode);
}
				...
				const unsigned long *advertising)
{
	...
	return mac->pcs->config(mac->pcs->dev, mode, interface, advertising);
}
				  ...
				  int speed, int duplex)
{
	...
	mac->pcs->link_up(mac->pcs->dev, mode, interface, speed, duplex);
}
			    ...
			    struct device_node *dpmac_node, int id)
{
	...
	mac->pcs = mdio_lynx_pcs_create(mdiodev);
	if (!mac->pcs) {
		netdev_err(mac->net_dev, "failed to create Lynx PCS device!\n");
		return -ENOMEM;
	}
...
{
	struct mdio_lynx_pcs *pcs = mac->pcs;

	if (mac->pcs) {
		put_device(&pcs->dev->dev);
		mdio_lynx_pcs_free(mac->pcs);
		mac->pcs = NULL;
	}
}
