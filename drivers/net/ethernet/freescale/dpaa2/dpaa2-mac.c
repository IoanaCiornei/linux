...
static irqreturn_t dpaa2_mac_irq_handler(int irq_num, void *arg)
{
	...
	struct dpaa2_mac *priv = dev_get_drvdata(dev);
	u32 status;
	int err;
...
static int dpaa2_mac_probe(struct fsl_mc_device *dpmac_dev)
{
	 ...
	 */
	peer_dev = fsl_mc_get_endpoint(dpmac_dev);
	if (!IS_ERR(peer_dev) && peer_dev->dev.type == &fsl_mc_bus_dpni_type) {
		return -EPROBE_DEFER;
	}
	...
	priv->mc_dev = dpmac_dev;
	priv->net_dev = netdev;

	SET_NETDEV_DEV(netdev, dev);

	snprintf(netdev->name, IFNAMSIZ, "mac%d", dpmac_dev->obj_desc.id);

	dev_set_drvdata(dev, priv);

	err = fsl_mc_portal_allocate(dpmac_dev, FSL_MC_IO_ATOMIC_CONTEXT_PORTAL,
				     &dpmac_dev->mc_io);
	if (err || !dpmac_dev->mc_io) {
		dev_dbg(dev, "fsl_mc_portal_allocate error: %d\n", err);
		err = -EPROBE_DEFER;
		goto free_netdev;
	}
	priv->mc_io = dpmac_dev->mc_io;

	// Not probing DPMAC that are TYPE_FIXED */
	if (dpaa2_mac_is_type_fixed(dpmac_dev, priv->mc_io)) {
		err = -EPROBE_DEFER;
		goto free_dpmcp;
	}

	// TODO: call dpmac_mac_connect(priv);
	...
	err = dpaa2_mac_setup_irqs(dpmac_dev);
	if (err) {
		err = -EFAULT;
		goto disconnect_mac;
	}

	err = register_netdev(priv->net_dev);
	if (err < 0) {
...
teardown_irqs:
	dpaa2_mac_teardown_irqs(dpmac_dev);
disconnect_mac:
	dpaa2_mac_disconnect(priv);
free_dpmcp:
	fsl_mc_portal_free(dpmac_dev->mc_io);
free_netdev:
	free_netdev(netdev);
	return err;
}

static int dpaa2_mac_remove(struct fsl_mc_device *dpmac_dev)
{
