...
void lynx_pcs_an_restart(struct phylink_pcs *pcs)
{
	struct lynx_pcs *lynx = phylink_pcs_to_lynx(pcs);

	switch (lynx->interface) {
	case PHY_INTERFACE_MODE_2500BASEX:
		break;
	default:
		dev_err(&lynx->mdio->dev, "Invalid PCS interface type %s\n",
			phy_modes(lynx->interface));
		break;
	}
}
					 ...
					 struct phylink_link_state *state)
{
	...
	// TODO: is this right?
	bmcr = mdiobus_read(bus, addr, MII_BMCR);
...
void lynx_pcs_get_state(struct phylink_pcs *pcs,
			struct phylink_link_state *state)
{
	struct lynx_pcs *lynx = phylink_pcs_to_lynx(pcs);
				   ...
				   const unsigned long *advertising)
{
	...
	mdiobus_c45_write(bus, addr, MDIO_MMD_VEND2, MII_ADVERTISE,
			  LPA_USXGMII_10000 | LPA_USXGMII_LINK |
			  LPA_USXGMII_FULL_DUPLEX |
			  ADVERTISE_SGMII | ADVERTISE_LPACK);
	return 0;
}

int lynx_pcs_config(struct phylink_pcs *pcs, unsigned int mode,
				       ...
				       int speed, int duplex)
{
	...
	if (duplex == DUPLEX_HALF)
		if_mode |= SGMII_IF_MODE_DUPLEX;
	if_mode |= SGMII_IF_MODE_SPEED(SGMII_SPEED_2500);
...
void lynx_pcs_link_up(struct phylink_pcs *pcs, unsigned int mode,
