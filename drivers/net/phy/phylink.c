...
static void phylink_resolve(struct work_struct *w)
{
	...
	if (mac_config) {
		if (link_state.interface != pl->link_config.interface) {
			 ...
			 * Force the link down and then reconfigure.
			 */
			if (cur_link_state)
 ...
 * code word.  Decode the USXGMII code word and setup the necessary fields
 * (speed, duplex) into the phylink_link_state structure.
 */
void phylink_decode_usxgmii_word(struct phylink_link_state *state,
				 uint16_t lpa)
{
	switch (lpa & LPA_USXGMII_SPD_MSK) {
	case LPA_USXGMII_10:
		state->speed = SPEED_10;
		break;
	case LPA_USXGMII_100:
		state->speed = SPEED_100;
		break;
	case LPA_USXGMII_1000:
		state->speed = SPEED_1000;
		break;
	case LPA_USXGMII_2500:
		state->speed = SPEED_2500;
		break;
	case LPA_USXGMII_5000:
		state->speed = SPEED_5000;
		break;
	case LPA_USXGMII_10000:
	...
	if (lpa & LPA_USXGMII_FULL_DUPLEX)
		state->duplex = DUPLEX_FULL;
	else
		state->duplex = DUPLEX_HALF;
}
